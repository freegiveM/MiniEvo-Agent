...
## 21. 反思信号的预算与过期（2026-09-09）

第 18.1 的回路 A 前置条件（消费账本）已在第 19 节修完，反馈不再被无关失败
烧掉。但"回流"这件事本身当时只做了一半：账本记了 decision 与 reason，回传
给生成器的信号是"这个根因上次被拒了，理由是某项受保护指标回退"。两个缺口，
都是这一节修的。

### 21.1 缺口一：信号说明不了"上次试的是哪个改法"

只有 decision + reason 时，生成器完全可以再提一遍等价的修改，门禁再拒一次，
`max_attempts_per_root_cause` 就这么烧完——而账本上看着一切正常，三次尝试
都有记录、都有理由。

修法：账本加 `edits_json` 列，存这次具体改了什么（`edits`）、分数怎么变的
（`score_before` / `score_after`）、哪几项受保护指标回退了
（`regressed_metrics`）。条目形状取自 SkillOpt 的 step buffer
（`skillopt/engine/trainer.py` 的 `_format_step_buffer`），它回传的正是
`### Step 12 — REJECT (7/40 failed)` 加 `Rejected edits (score 0.62 → 0.55)`。

**没跑到评测的尝试不填 0.0，那几个键直接不出现。** 一条没测过的尝试与一条
测出 0.0 的尝试意义完全相反：填 0 会让生成器以为上次那个改法把分数打到了
零，从而绕开一个其实没被检验过的方向。与 `_empty_metrics` 里 CI 用 None、
`gates.evaluation_success` 用三态是同一条纪律。

顺带把 `regressed` 指标名从 `reason` 那句自然语言里提出来，`_propose` 现在
返回结构化的 `non_regression`。原先想按指标名做事的调用方只能解析字符串，
而解析出来的东西会随文案改动静默失效。

### 21.2 缺口二：被拒的 diff 永不过期

一条针对 v1 提示词被拒的记录，在提示词走到 v6 之后还在被回放。那句"别这么
改"是相对一个已经不存在的基线得出的——当前基线里可能压根没有那段文本。
继续回传它既挤占输入预算，又把生成器往一个已经无效的禁区上引。

修法：账本加 `baseline_version` 列，记**门禁基线**（active 版本，不是
`parent_version`——亲本只决定候选从哪改起，不影响结论还成不成立）。新增
`list_reflection_attempts` 只取同一基线上的行。老行（本列加入之前写的）
`baseline_version` 为 NULL，同样不回传：它们归属不到任何基线，猜一个基线号
会把陈旧结论伪装成新鲜的。

SkillOpt 是同一个做法，而且更彻底：它的 `step_buffer` 建在 epoch 循环
**内部**（`trainer.py:1151`），epoch 一结束就整个丢掉。

**这里有一条必须写死的纪律：过期只作用于反思信号，绝不作用于重试上限。**
`count_attempts_by_fingerprint` 照旧数全部历史行。那个数回答的是"在这个根因
上一共花过多少次全量回放"，那笔钱不会因为提示词换了版本就退回来。让它跟着
基线过期的话，每激活一个新版本就等于给所有根因重置额度，
`max_attempts_per_root_cause` 名存实亡——又一道假装在工作的门禁，本仓库
反复出现的那个形状。`test_expiry_does_not_refund_the_retry_budget` 钉住它。

同理，`list_evolution_attempts`（审计视图）不做任何过滤。过期作用于"喂回
生成器的信号"，不作用于"能不能看见"。

### 21.3 单轮回传条数上限

账本随轮次单调增长，而它整条进候选生成那次调用的**输入**。
`EVOAGENT_EVOLUTION_GENERATOR_TOKEN_BUDGET` 封的是 `max_tokens`，也就是
输出，封不住输入。所以不设上限的后果不是报错，是反思信号把 `failure_cases`
和 `active_prompt` 挤出输入，而且看不出来。

新增 `EVOAGENT_EVOLUTION_MAX_REFLECTION_ATTEMPTS`，默认 12。原先生成器内部
硬编码 `[:20]`（`evolution_v2.generate`），那个数字没有依据且切在错误的层：
回传几条是调用方的预算决定，生成器自己再切一刀会让调用方报出的"送了 N 条"
与实际送进提示词的条数不一致，而那个数是唯一能看出信号有没有被丢的地方。
已删除。

排序在截断之前：先按"这个根因一共试过几次"降序（试得最多的那个最接近重试
上限，最该被劝阻），再按账本行倒序。截断掉的条数进
`reflection.dropped_over_budget`，同时出现在返回值和落盘的
`structured_candidate` 里。静默切会让"这轮只有 2 条历史"和"这轮有 40 条但
只送了 2 条"长得一样。

### 21.4 这一节没有解决什么

- **回路 B / C 依旧不存在。** 这一节让回流的信号变得可用且有界，它不能让
  候选通过门禁。第 20.3 节那个"候选分数持平"的结论没有变化。
- **反思信号的有效性没有实测。** 上面全部是结构性论证（信号不足会导致重复
  提案、陈旧信号会误导生成器），不是"接了这个之后候选质量提升了 X"。要拿到
  后者需要同一批反馈在开关两侧各跑一轮，而当前 holdout 只有 1 条缺陷样本
  （第 18.2 节），量不出来。
- **`edits` 取的是 `prompt_additions`**，不是完整 diff。delta 路径退化成
  `add focus-rule X` 的列表。够生成器分辨"上次试的是不是这个改法"，不够
  精确重放一次候选——那需要落 `candidate_version` 再去 `skill_versions`
  取回提示词，留待需要时再做。
